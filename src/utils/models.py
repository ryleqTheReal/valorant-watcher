...
class PresenceResponse:
    ...
    def __post_init__(self) -> None:
        if self.presences and isinstance(self.presences[0], dict):
            self.presences = [Presence(**_filter_known(Presence, p)) for p in self.presences]  # pyright: ignore[reportCallIssue, reportArgumentType]
...
class PresenceWebsocketEvent(WebsocketEventWrapper[WebsocketEventEnvelope[PresenceResponse]]):
    ...
    def from_raw_string(cls, raw: str) -> PresenceWebsocketEvent:
        ...
        presence_response = PresenceResponse(**_filter_known(PresenceResponse, payload.get("data", {})))  # pyright: ignore[reportCallIssue, reportUnknownMemberType, reportArgumentType]
...
class _XPHistoryEntry:
    ...
    def __post_init__(self) -> None:
        ...
        if self.XPSources and isinstance(self.XPSources[0], dict):
            self.XPSources = [_XPSource(**_filter_known(_XPSource, s)) for s in self.XPSources]  # pyright: ignore[reportCallIssue, reportArgumentType]
        if self.XPMultipliers and isinstance(self.XPMultipliers[0], dict):
            self.XPMultipliers = [_XPMultiplier(**_filter_known(_XPMultiplier, m)) for m in self.XPMultipliers]  # pyright: ignore[reportCallIssue, reportArgumentType]
        ...
        if self.SeasonalInfoBySeasonID:
            ...
            if isinstance(first_val, dict):
                self.SeasonalInfoBySeasonID = {
                    k: _SeasonalInfo(**_filter_known(_SeasonalInfo, v)) for k, v in self.SeasonalInfoBySeasonID.items()  # pyright: ignore[reportCallIssue, reportArgumentType]
                }
...
class PlayerMMRResponse:
    ...
    def __post_init__(self) -> None:
        ...
        if self.QueueSkills:
            ...
            if isinstance(first_val, dict):
                self.QueueSkills = {
                    k: _QueueSkillData(**_filter_known(_QueueSkillData, v)) for k, v in self.QueueSkills.items()  # pyright: ignore[reportCallIssue, reportArgumentType]
                }
...
class MatchDetailEvent:
    """Payload for Event.MATCH_DETAIL_FETCHED.

    Emitted whenever the match-details endpoint completes (success or failure).
    riot_status == 0 indicates a transport/exception failure, not an HTTP status.
    """
    shard: str
    match_id: str
    riot_status: int
...
class MatchHistoryEvent:
    """Payload for Event.MATCH_HISTORY_FETCHED.

    Emitted once a full match-history assembly (across pages) is complete or aborted.
    """
    shard: str
    puuid: str
    riot_status: int
    match_history: dict[str, Any] | None
    fetch_time_ms: int


@dataclass
class IngameLoadoutsEvent:
    """Payload for Event.INGAME_LOADOUTS_FETCHED.

    Carries the match_id captured at fetch time so downstream submitters
    don't race against gamestate transitions that clear active_match_id.
    """
    match_id: str
